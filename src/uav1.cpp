...
int main(int argc, char **argv)
{
	...
	ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("uav1/mavros/state", 1, state_cb);
	ros::Subscriber loc_sub = nh.subscribe<geometry_msgs::PoseStamped>("uav1/mavros/local_position/pose", 1, loc_cb);
	...
	ros::ServiceClient mode_client = nh.serviceClient<mavros_msgs::SetMode>("uav1/mavros/set_mode");
	...
	while(current_loc.pose.position.x >= -9.8 && ros::ok() && nh.getParam("uav1_ver", permission1))
		{
		...
		else if( !current_state.armed && current_state.guided && permission1)
			{
	...
	while(ros::ok())
		{
		if(current_loc.pose.position.x <=-1.2)
			{
			local_pos_pub.publish(pose);
			ROS_INFO("Return to Home");
			}
		if(current_loc.pose.position.x >=-1.2 && current_loc.pose.position.z >= 9.8)
			{
			set_mode.request.custom_mode = "LAND";
			if( mode_client.call(set_mode) && set_mode.response.mode_sent)
				{
				ROS_INFO("Mode -> LAND");
				nh.setParam("uav1_ver", false);
				break;
				}
			}
