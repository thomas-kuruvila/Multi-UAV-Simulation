...
int main(int argc, char **argv)
{
	...
	ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("uav0/mavros/state", 1, state_cb);
	ros::Subscriber loc_sub = nh.subscribe<geometry_msgs::PoseStamped>("uav0/mavros/local_position/pose", 1, loc_cb);
	...
	ros::ServiceClient mode_client = nh.serviceClient<mavros_msgs::SetMode>("uav0/mavros/set_mode");
	ros::ServiceClient takeoff_client = nh.serviceClient<mavros_msgs::CommandTOL>("uav0/mavros/cmd/takeoff");
	...
	while(current_loc.pose.position.x <= 9.6 && ros::ok() && nh.getParam("uav0_ver", permission0))
		{
		...
		else if( !current_state.armed && current_state.guided && permission0)
			{
		...
		else if(current_loc.pose.position.z < 9.6 && current_state.guided && current_state.armed)
			{
		...
		else if (current_loc.pose.position.z >=9.6)
			{
	...
	while(ros::ok())
		{
		if(current_loc.pose.position.x >=1.4)
			{
			local_pos_pub.publish(pose);
			ROS_INFO("Return to Home");
			}
		if(current_loc.pose.position.x <=1.4 && current_loc.pose.position.z >= 9.6)
			{
			set_mode.request.custom_mode = "LAND";
			if( mode_client.call(set_mode) && set_mode.response.mode_sent)
				{
				ROS_INFO("Mode -> LAND");
				nh.setParam("uav0_ver", false);
				break;
				}
			}
