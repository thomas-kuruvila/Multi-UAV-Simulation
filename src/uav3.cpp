...
{
	ros::init(argc, argv, "uav3_node");
	ros::NodeHandle nh;

	ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("uav3/mavros/state", 1, state_cb);
	ros::Subscriber loc_sub = nh.subscribe<geometry_msgs::PoseStamped>("uav3/mavros/local_position/pose", 1, loc_cb);
	ros::Publisher local_pos_pub = nh.advertise<geometry_msgs::PoseStamped>("uav3/mavros/setpoint_position/local", 10);
	ros::ServiceClient arming_client = nh.serviceClient<mavros_msgs::CommandBool>("uav3/mavros/cmd/arming");
	ros::ServiceClient mode_client = nh.serviceClient<mavros_msgs::SetMode>("uav3/mavros/set_mode");
	ros::ServiceClient takeoff_client = nh.serviceClient<mavros_msgs::CommandTOL>("uav3/mavros/cmd/takeoff");
	...
	while(current_loc.pose.position.y >= -9.8 && ros::ok() && nh.getParam("uav3_ver", permission3))
		{
		...
		else if( !current_state.armed && current_state.guided && permission3)
			{
